...
func getRequest(body map[string]interface{}) (Response, error) {
	...
	// remove all other keys other than file or environment from body
	for key := range body {
		if key != "files" && key != "environment" {
			delete(body, key)
		}
	}
	...
	log.Debug().Interface("CODE SANDBOX BODY", body).Msg("Debugging code sandbox body...")
